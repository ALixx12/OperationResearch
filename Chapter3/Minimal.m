...
% Supply 为供应量或产地产量(列向量)；Demand 为需求量或销量(列向量)
% 输出项
% X 为运输分配方案;b 记录了数字格(1 表示对应的是基变量，数字格，0 表示非基变量)
function [X,b]=Minimal(Cost,Supply,Demand)
...
while flag==0
    ...
    [TT,kk]=min(T);
    %Cost(kk,J(kk))元素是Cost 中最小者。于是考虑Supply(kk)与Demand(J(kk))之
    %间的关系并安排运输方案。若第J(kk)个销地的销量超过第kk 个产地的产量，则
    %将第kk 个产地的产量Supply(kk)全部供应给第J(kk)个销地，然后将Cost中的第kk
    %行划去(通过赋值inf 完成)。若第kk 个产地的产量超过第J(kk)个销地的销量，
