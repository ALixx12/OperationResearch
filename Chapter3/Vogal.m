...
% Supply 为供应量或产地产量(列向量)；Demand为需求量或销量(列向量)
% 输出项
% X 为运输分配方案;b 记录了数字格(1表示对应的是基变量，数字格，0表示非基变量)
...
while flag==0
    ...
    for k=1:m %求出每行最小元素和次最小元素的列标J1(k)和J2(k)
        [T1(k),J1(k)]=min(TRC(k,:));TRC(k,J1(k))=inf;[T2(k),J2(k)]=min(TRC(k,:));
    end
    for s=1:n %求出每列最小元素和次最小元素的列标JJ1(s)和JJ2(s)
        [TT1(s),JJ1(s)]=min(TCC(:,s));TCC(JJ1(s),s)=inf;[TT2(s),JJ2(s)]=min(TCC(:,s));
    end
    ...
    %差额最大者在kk1 行，该行最小元素为第J1(kk1)个元素
    ...
    %行划去(通过赋值inf 完成)。若第kk个产地的产量超过第Jkk 个销地的销量，
    %则第kk个产地的产量全部满足第Jkk个销地。然后将Cost 中的第Jkk列划去(通过赋值inf完成)
