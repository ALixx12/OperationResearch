...
% min sum(c(ij)x(ij))
% s.y.
% sum(x(ij))=bi,i=1,2,...n(n 个产地)
% sum(x(ij))=ai,i=1,2,...m(m 个销地)
% x(ij)>=0
% 若sum(bi)=sum(aj),则为产销平衡问题
% 若sum(bi)>=sum(aj),则为销大于产问题
% 若sum(bi)<=sum(aj),则为产大于销问题
% 输入项
% Cost 为单位运费表(矩阵)，行对应产地，列对应销地
% 若无法从某产地出发到达某销地，将相应的单位运费取做某个很大的正数，如10^5
% Demand 为销地需求量(列向量)；Supply 为产地供应量或产量(列向量)；
% 输出项
% F 为最有运费；X 为最优运输分配方案
...
if Temp>0
    disp('这是销大于产的运输问题。最优运输分配方案及最有运费F 为：')
    Cost(m+1,:)=zeros(1,n);Supply(m+1)=Temp;m=m+1;
elseif Temp<0
    disp('这是产大于销的运输问题。最优运输分配方案及最有运费F 为：')
    Cost(:,n+1)=zeros(m,1);Demand(n+1)=-Temp;n=n+1;
else
    disp('这是产销平衡的运输问题。最优运输分配方案及最有运费F 为：')
end
mn=m*n;A=zeros(m+n,m*n); %这里求出运输问题数学模型的约束矩阵A
for k=1:m
    A(k,((k-1)*n+1):(k*n))=1; %这是矩阵A 的第k 行，对应着第k 个产地
end
T=1:n:mn;
for k=1:n
    A((k+m),(T+k-1))=1;%这里对应着销地
end
%下面通过Vogal 法求出初始基可行解(初始运输方案)，也可以改为西北角法或最小元素法
...
% 为避免某些产销不平衡问题的单位运费矩阵中出现inf 和在计算总运费时出现NaN,作如下处理
for i=1:m
    for j=1:n
        if Cost(i,j)==inf
            Cost(i,j)=0;
