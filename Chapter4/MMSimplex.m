...
% 输出项：
% xstar 为最优解;fxstar 为最优值；iter 为迭代次数; A0 为最终单纯形表
% IB 为最优基变量下标;IN 为非基变量下标；SA 为松弛变量下标
% xSA 为松弛量取值;sigma 为最终检验数(可不显示)
% A0 为最终单纯性表(第一列为b);
% InverseOfB=inv(B)为基矩阵B 的逆矩阵(用于灵敏度分析)。输出项按需要选择
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
% IB 记录了原问题系数矩阵有多少个E(:,i)，即m‐length(s)个,对应的x(i)为初始
% 基变量。IR 则记录了原问题系数矩阵缺少的E(:,i)下标，即i，这些是需要通过人
% 工变量补齐的共length(s)个人工变量，这些变量也是初始基变量。SA 记录松弛
% 变量(剩余变量)下标IB 记录了基变量的下标，而IR 记录了人工变量的下标(共有
% length(s)个人工变量)。退出时矩阵A0 具有一个单位子矩阵，可能含有人工变
...
while (length(IR)~=0)&(flag==0) %这表明有人工变量才需要求解第一阶段问题
    ...
    while t~=0
        ...
        if kk==0
            ...
            IN(jj)=TT; x(IB)=A0(:,1)'; IN(find(IN==0))=[]; x(IN)=zeros(1,length(IN));
            cB=c0(IB); %新的基可行解及价值系数
            sigma=c0-cB'*A0(:,2:n+length(s)+1); t=length(find(sigma>0));
            %再次计算检验数并假设检验数中有t 个大于零的检验数
        end
        k=k+1;
    end
    if sum(x(IR))~=0
...
if (flag==1)|(flag==2)
    ...
    IB;N=1:n; N(IB)=[]; IN=N; IN(find(IN==0))=[];x(IN)=zeros(1,length(IN)); cB=c(IB);
    A0=A0(:,1:n+1); %回到原问题的有关矩阵和向量
    sigma=c'-cB'*A0(:,2:n+1); t=length(find(sigma>0));
    %计算原问题的检验数并假设检验数中有t 个大于零的检验数
    while (t~=0)&(flag==0)
        [sigmaJ,jj]=max(sigma);
        %jj 是sigma 中绝对值最大者所在列，即A0中的第jj+1 列(A0中第一列为b)，该列对应
        %的非基变量x(jj)为换入变量，而sigmaJ 则是相应的检验数
        tt=find(A0(:,jj+1)>0);kk=length(tt);
        %检查增广系数矩阵A0 中第j+1 列元素是否有大于零的元素
        if kk==0
            disp('原问题为无界解');
            xstar=[];fxstar=[];A0=[];IB=[];
            iter=k; flag=1;
            ...
            [thetaI,ii]=min(theta); Temp=tt(ii);
            %比值最小的theta 值，选择换出变量。这时A0(Temp,jj+1)为旋转主元
            for i=1:m
                if i~=Temp
                    A0(i,:)=A0(i,:)-(A0(Temp,:)/A0(Temp,jj+1))*A0(i,jj+1);
                else
