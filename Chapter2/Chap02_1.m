...
%? 矩阵 A 中有一个单位子矩阵，不需要引入人工变量。非基变量检验数满足最 优性规则，但资源向量有负值
function[xstar,fxstar,A0,IB,iter]=Dismplex(A,b,c)
[m,n]=size(A);E=eye(m);IB=zeros(1,m);k=0;
for i=1:m
     for j=1:n
          if A(:,j)==E(:,i)
               IB(i)=j;SA(i)=j; %?IB记录基变量下标，SA 记录松弛变量下标
          elseif A(:,j)==(-E(:,i))
...
%IN 为非基变量下标
sigma=c'-cB' * A0(:,2:n+1);
%计算检验数，由于对偶单纯形法的要求，这里 sigma 全部小于或等于零
t=find(A0(:,1)<0);%检查 b 列的负数
flag=0;
while (t~=0)&(flag==0)
     [bjj,jj]=min(A0(:,1));
     %? 这里的 jj 是 A0(:,1)中绝对值最大者所在行，即 A0 中的第 jj 行，该行对应的基 变量
     %?x(IB(jj))为换出变量，而 bjj 则是相应的数值
     tt=find(A0(jj,2:n+1)<0);kk=length(tt);
     %? 检查增广系数矩阵 A0 中第 jj 行元素是否有小于零的元素
     if kk==0
         disp('原问题无解')
         ...
         flag=1;
          ...
          theta=zeros(1,kk);
           ...
           %? 比值最小的 theta 值，选换出变量。这时 x(Temp)为换入变量。A0(jj,Temp+1) 为旋转主元
            for i=1:m
                if i ~=jj
