...
% 首先调用 SenSimplex.m 计算该线性规划问题，将得到所需的相关信息
[m,n]=size(A);
[IB,IN,SA,IBB,INN,InverseOfB,A0,sigma]=SenSimplex(A,b,c)
% IB 记录的是基变量下标，IN 记录的是非基变量下标，SA 记录的是松弛变量下标
% IBB 记录的是除去松弛变量后的基变量下标，INN 记录的除去松弛变量后的非基变量下标
disp('以上显示的是原问题的最优解(xstar)，最优值(fxstar)，迭代次数(iter)，最优 基下标(IB)等信息。')
for r=1:length(IN)
...
Deltac=input('按照列向量输入价值系数的变化（一次只能有一个 c 变化且输入的 应该是 Deltac）')
