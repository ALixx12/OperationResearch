...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%
%?IB 记录了原问题系数矩阵有多少个 E(:,i)，即m‐length(s)个,对应的 x(i)为初始
%? 基变量。IR则记录了原问题系数矩阵缺少的E(:,i)下标，即i，这些是需要通过人
%? 工变量补齐的共length(s)个人工变量，这些变量也是初始基变量。SA 记录松弛
 %?变量(剩余变量)下标 IB记录了基变量的下标，而 IR记录了人工变量的下标(共有
...
while (length(IR)~=0)&(flag==0) %这表明有人工变量才需要求解第一阶段问题
        ...
        while t~=0
               [sigmaJ,jj]=max(sigma);
               %这里的 jj 是 sigma 中绝对值最大者所在列，即 A0 中的第 jj+1 列(A0 中第 一列为 b)，对应的非基
               %变量x(jj)为换入变量，而sigmaJ则是相应的检验数
               tt=find(A0(:,jj+1)>0); kk=length(tt);
               %检查增广系数矩阵 A0中第jj+1列元素是否有大于零的元素
               if kk==0
                       disp('原问题为无界解'); %即A0的第 jj+1列元素全部小于或等于零
				...
				if sum(x(IR))~=0
					disp('原问题无解');%此时没有检验数小于零，但第一阶段有最优解，从而 原问题无解
					xstar=[];fxstar=[];A0=[];IB=[];iter=k;
...
if (flag==1)|(flag==2)
    ...
    N(IB)=[];
    IN=N;
    IN(find(IN==0))=[];x(IN)=zeros(1,length(IN));
    cB=c(IB);
	...
	while (t~=0)&(flag==0)
		[sigmaJ,jj]=max(sigma);
		%jj是 sigma 中绝对值最大者所在列，即 A0 中的第 jj+1 列(A0 中第一列为 b)，该列对应
        %的非基变量x(jj)为换入变量，而 sigmaJ则是相应的检验数
		tt=find(A0(:,jj+1)>0);kk=length(tt);
		%检查增广系数矩阵 A0中第j+1列元素是否有大于零的元素
		if kk==0
			disp('原问题为无界解');
			xstar=[];fxstar=[];A0=[];IB=[];
			iter=k; flag=1;
